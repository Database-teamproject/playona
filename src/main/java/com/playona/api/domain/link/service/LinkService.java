...
public class LinkService {
    ...
    public LinkResponse createLink(String url) {
        Track track = youtubeTrackService.getTrackFromUrl(url);
        ...
        if (existing.isPresent()) {
            List<PlatformTrack> existingPlatforms = platformTrackRepository.findByTrack(existing.get().getTrack());
            return new LinkResponse(existing.get(), baseUrl, existingPlatforms);
        }
        ...
        List<PlatformTrack> matchedPlatforms = trackMatchingService.matchAll(track);

        return new LinkResponse(sharedLink, baseUrl, matchedPlatforms);
    }
    ...
    public String getRedirectUrl(String shortCode, String userUuid) {
        ...
        if (user != null) {
            List<UserPlatformPreference> prefs = userPlatformPreferenceRepository.findByUserOrderByPriorityAsc(user);
